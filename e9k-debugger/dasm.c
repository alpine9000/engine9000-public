// Disassembly cache manager implementation
#include "dasm.h"
#include "debugger.h"
#include "debug.h"
#include "elfutil.h"
...
typedef struct dasm_cache {
    char               **lines;   // instruction text (post-colon)
    uint64_t            *addrs;   // addresses for each line
    int                  n;
    int                  cap;
    int                  addr_hex_width; // 8 or 16 typically
    int                  ready;
} dasm_cache_t;

static dasm_cache_t g_dasm;

static void dasm_clear(void)
{
    if (g_dasm.lines) {
...
static void dasm_push(uint64_t addr, const char *text)
{
    if (!text) {
        text = "";
    }
...
static int parse_disasm_line(const char *ln, uint64_t *out_addr, const char **out_text)
{
    if (!ln || !*ln) {
        return 0;
    }
...
{
    // Get .text range directly from ELF file
    uint64_t lo=0, hi=0;
    ...
    while ((read = getline(&line, &cap, pipe)) != -1) {
        ...
        if (parse_disasm_line(line, &a, &text)) {
            dasm_push(a, text);
        }
    }
    if (line) {
...
static int lower_bound_addr(uint64_t addr)
{
    int lo = 0, hi = g_dasm.n; // [lo, hi)
    while (lo < hi) {
        int mid = lo + (hi - lo)/2;
